/**
 * ALC POS System - Automated Google Sheets Setup Script
 *
 * This script automatically creates and populates all 7 required sheets with:
 * - Headers
 * - Initial data
 * - 119 products
 * - Sample promo codes
 ...
 * 4. Click the "Run" button and select "setupALCPOSSystem"
 * 5. Authorize the script when prompted
 * 6. Wait 30-60 seconds for completion
 * 7. Check your sheet - all tabs should be created and populated!
 */

function setupALCPOSSystem() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();

  Logger.log('🚀 Starting ALC POS System Setup...');
  ...
  // Format and protect sheets
  formatSheets(ss);

  Logger.log('✅ Setup complete! Your ALC POS System is ready.');
  Logger.log('📝 Next steps:');
  Logger.log('   1. Go to Locations tab and add your Authorize.net credentials');
  Logger.log('   2. Review Products tab and adjust prices if needed');
  Logger.log('   3. Continue with DEPLOYMENT.md Step 2');
...
/**
 * Create and populate Products sheet with all 119 SKUs
 */
function setupProductsSheet(ss) {
  ...
  Logger.log(`  ✅ Products sheet created with ${products.length} products`);
}

/**
 * Create Transactions sheet (empty, just headers)
 */
function setupTransactionsSheet(ss) {
  Logger.log('  💳 Setting up Transactions sheet...');
  ...
  const headers = [
    'TransactionID', 'Timestamp', 'InvoiceNumber', 'CustomerID', 'CustomerName',
    'ProviderInitials', 'CollectorName', 'Items', 'Subtotal', 'PromoCode',
    'Discount', 'PriceOverrides', 'CashAmount', 'CardAmount', 'Total',
    'PaymentStatus', 'AuthNetTransID', 'LocationID', 'Description', 'EmailReceipt'
  ];
...
/**
 * Create Customers sheet (empty, just headers)
 */
function setupCustomersSheet(ss) {
  Logger.log('  👥 Setting up Customers sheet...');
  ...
  const headers = [
    'CustomerID', 'PatientID', 'FirstName', 'LastName', 'Email',
    'Phone', 'FirstVisit', 'LastVisit', 'TotalTransactions',
    'LifetimeValue', 'Notes'
  ];
...
/**
 * Create InvoiceTracking sheet with initial data
 */
function setupInvoiceTrackingSheet(ss) {
  Logger.log('  🧾 Setting up InvoiceTracking sheet...');

  let sheet = ss.getSheetByName('InvoiceTracking');
  if (!sheet) {
    sheet = ss.insertSheet('InvoiceTracking');
  } else {
    sheet.clear();
  }
  ...
  Logger.log('  ✅ InvoiceTracking sheet created');
}

/**
 * Create PromoCodes sheet with sample data
 */
function setupPromoCodesSheet(ss) {
  Logger.log('  🏷️  Setting up PromoCodes sheet...');

  let sheet = ss.getSheetByName('PromoCodes');
  if (!sheet) {
    sheet = ss.insertSheet('PromoCodes');
  } else {
    sheet.clear();
  }

  const headers = [
    'PromoCode', 'DiscountType', 'DiscountValue', 'Active', 'StartDate',
    'EndDate', 'UsageLimit', 'TimesUsed', 'ApplicableCategories', 'ApplicableSkus'
  ];
  ...
  Logger.log('  ✅ PromoCodes sheet created with sample codes');
}
...
function setupLocationsSheet(ss) {
  Logger.log('  📍 Setting up Locations sheet...');
  ...
  const headers = [
    'LocationID', 'LocationCode', 'LocationName', 'Address', 'City',
    'State', 'Zip', 'Active', 'AuthNetAPILogin', 'AuthNetTransKey',
    'AuthNetSignatureKey', 'AuthNetEnvironment'
  ];

  const data = [
    [
      'LOC-001', 'LOC', 'Main Clinic', '123 Main St', 'Your City',
      'CA', '12345', true, 'PASTE_API_LOGIN_HERE', 'PASTE_TRANS_KEY_HERE',
      'PASTE_SIGNATURE_KEY_HERE', 'SANDBOX'
    ]
  ];
  ...
  // Highlight cells that need to be filled in
  sheet.getRange(2, 9, 1, 3).setBackground('#fff3cd');
  sheet.getRange(2, 9, 1, 3).setNote('⚠️ IMPORTANT: Replace with your Authorize.net credentials from sandbox.authorize.net');

  Logger.log('  ✅ Locations sheet created');
  Logger.log('  ⚠️  Remember to add your Authorize.net credentials!');
}

/**
 * Create Config sheet with all settings
 */
function setupConfigSheet(ss) {
  Logger.log('  ⚙️  Setting up Config sheet...');
...
function testSetupComplete() {
  ...
  const requiredSheets = [
    'Products', 'Transactions', 'Customers', 'InvoiceTracking',
    'PromoCodes', 'Locations', 'Config'
  ];
  ...
  requiredSheets.forEach(sheetName => {
    ...
    if (sheet) {
      ...
      results.push(`✅ ${sheetName}: ${rowCount} rows`);
    } else {
      results.push(`❌ ${sheetName}: MISSING`);
      allPresent = false;
    }
  });
  ...
  if (allPresent) {
    Logger.log('\n✅ All sheets present and ready!');
  } else {
    Logger.log('\n❌ Some sheets are missing. Run setupALCPOSSystem() again.');
  }
